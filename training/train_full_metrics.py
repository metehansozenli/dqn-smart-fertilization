"""
IEEE makale figürleri için kapsamlı metriklerle tam DQN eğitimi.
Loglananlar: ödül, verim, kayıp (loss), epsilon, eylem dağılımı.
"""
...
def train_full(episodes=800, seed=42):
    ...
    print(f"Hibrit DQN Eğitimi | {episodes} bölüm | {action_dim} eylem")
    for ep in tqdm(range(episodes)):
        ...
        if (ep + 1) % 100 == 0:
            print(f"Bölüm {ep+1:4d} | Ödül: {np.mean(rewards[-50:]):7.1f} | "
                  f"Verim: {np.mean(yields[-50:]):6.0f} | Kayıp: {np.mean(losses[-50:]):.4f} | eps: {agent.epsilon:.3f}")
    ...
    ax1.set_title('(a) Bölüm Ödülü', fontsize=12, fontweight='bold')
    ax1.set_xlabel('Bölüm (Episode)')
    ax1.set_ylabel('Kümülatif Ödül')
    ...
    ax2.axhline(3800, color='gray', ls='--', lw=1.5, label='Veri Medyanı')
    ax2.set_title('(b) Final Verimi (kg/ha)', fontsize=12, fontweight='bold')
    ax2.set_xlabel('Bölüm (Episode)')
    ...
    ax3.set_title('(c) TD Kaybı (Smooth L1)', fontsize=12, fontweight='bold')
    ax3.set_xlabel('Bölüm (Episode)')
    ax3.set_ylabel('Kayıp (Loss)')
    ax3.set_yscale('log')
    ...
    ax4.set_title('(d) Keşif Oranı (ε)', fontsize=12, fontweight='bold')
    ax4.set_xlabel('Bölüm (Episode)')
    ax4.set_ylabel('ε')
    ax4.set_ylim(0, 1.05)
    ax4.grid(True, alpha=0.3)

    # 5. Eylem Dağılımı (eğitimin son %30'u)
    ...
    ax5.set_title('(e) Eylem Dağılımı (Son %30 Bölüm)', fontsize=12, fontweight='bold')
    ax5.set_ylabel('Sayı')
    ax5.grid(True, alpha=0.3, axis='y')

    # 6. Verime karşı Gübre verimliliği (son bölümlerden saçılım)
    ax6 = fig.add_subplot(3, 2, 6)
    # Verimlilik eğilimini simüle et
    late_y = yields[int(len(yields)*0.5):]
    ax6.hist(late_y, bins=25, color='#64B5CD', edgecolor='black', alpha=0.75)
    ax6.axvline(np.mean(late_y), color='red', ls='--', lw=2, label=f'Ortalama: {np.mean(late_y):.0f}')
    ax6.set_title('(f) Verim Dağılımı (İkinci Yarı)', fontsize=12, fontweight='bold')
    ax6.set_xlabel('Verim (kg/ha)')
    ...
    plt.close()
    print(f"Kapsamlı figür kaydedildi → {out}")

    # Özet
    print("\n========== FİNAL METRİKLERİ ==========")
    print(f"Bölümler (Episodes)     : {episodes}")
    print(f"Final Ort Ödül (50)     : {np.mean(rewards[-50:]):.1f}")
    print(f"Final Ort Verim (50)    : {np.mean(yields[-50:]):.0f} kg/ha")
    print(f"En İyi Verim            : {np.max(yields):.0f} kg/ha")
    print(f"Final Ort Kayıp         : {np.mean(losses[-50:]):.5f}")
    print(f"Final Epsilon           : {agent.epsilon:.3f}")
    print(f"En çok kullanılan eylem : {action_names[np.argmax(values)]}")
